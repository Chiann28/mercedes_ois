...
// Controller Start
app.controller("AdjustmentsController", function ($scope, API) {
  ...
  $scope.DoAdjustment = function (transaction_id) {
    const tx = $scope.Transactions.find(
      (t) => t.transaction_id === transaction_id
    );

    if (tx) {
      $scope.selectedTransaction = angular.copy(tx);
      $scope.adjustment = { amount: null, remarks: "" };
  ...
  $scope.SaveAdjustment = function () {
    if (!$scope.adjustment.amount || isNaN($scope.adjustment.amount)) {
      Swal.fire({
        icon: "warning",
        title: "Invalid Input",
    ...
    API.postApi("api/AdjustmentsAPI.php", data).then(function (response) {
      var final_response = JSON.parse(atob(response.data));
